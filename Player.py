...
class Player:
    ...
    #X = "/Users/janet/Desktop/TicTacToe/x.png"
    #O = "/Users/janet/Desktop/TicTacToe/o.png"
    X = "X"
    O = "O"
    human = False

    def __init__(self, human, player, name):
        self.human = human
        self.name = name
        self.id = player
        if player == 1:
    ...
    def get_winning_coords(self, mainboard):
        status = mainboard
        sums = [sum(status[0:3]), sum(status[3:6]), sum(status[6:9]), sum(status[0::3]), sum(status[1::3]),
                sum(status[2::3]), sum(status[::4]), sum(status[2:-2:2])]

        index = [index for index, value in enumerate(sums) if value == 3 or value == -3]
        return index
...
class AIbot(Player):
    ...
    def __init__(self, player):
        Player.__init__(self,False, player, self.name)
